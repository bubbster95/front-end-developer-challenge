...
function FullPath({ path, index }) {
  ...
  return (
    ...
    <div className={`full-path ${index === 0 ? "area-a" : "area-b"}` } data-testid='full-path'>
      ...
      {path.runesOnPath.map((runeId, index) => {
        ...
        return <Rune key={`rune-${runeId}`} rune={rune} data-testid={`rune-${runeId}`}/>;
      })}
      <GrayBar runesOnPath={runesOnPath} data-testid="gray-bar"/>
    </div>
  );
}
